...
class DualEncoderEpsNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.grad_local_dist_mlp = MultiLayerPerceptron(2 * config.hidden_dim, [config.hidden_dim, config.hidden_dim // 2, 1], activation=config.mlp_act)
        ...
        if self.model_type == "diffusion":
            ...
            sigmas = torch.tensor(np.exp(np.linspace(np.log(config.sigma_begin), np.log(config.sigma_end), config.num_noise_level)), dtype=torch.float32)
    ...
    def langevin_dynamics_sample(
        ...
        **kwargs
        ...
        if self.model_type == "diffusion":
            ...
            return self.langevin_dynamics_sample_dsm(
                atom_type,
                pos_init,
                bond_index,
                bond_type,
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc="sample"):
                ...
                if sampling_type == "generalized" or sampling_type == "ddpm_noisy":
                    ...
                    if sampling_type == "generalized":
                        ...
                        mean_eps = ((atm1.sqrt() * beta_t) * pos0_from_e + ((1 - beta_t).sqrt() * (1 - atm1)) * pos) / (1.0 - at)
                        mean = mean_eps
                        mask = 1 - (t == 0).float()
                        logvar = beta_t.log()
    ...
    def get_loss_dsm(
        ...
        if self.config.edge_encoder == "gaussian":
            ...
            d_sgn = torch.sign(d_perturbed)
            d_perturbed = torch.clamp(d_perturbed * d_sgn, min=0.01, max=float("inf"))
        d_target = 1.0 / (sigmas_edge**2) * (d_gt - d_perturbed)  # (E_global, 1), denoising direction
    ...
    def langevin_dynamics_sample_dsm(
        self,
        atom_type,
        pos_init,
        bond_index,
